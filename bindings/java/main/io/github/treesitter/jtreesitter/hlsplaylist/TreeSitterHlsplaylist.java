package io.github.treesitter.jtreesitter.hlsplaylist;

import java.lang.foreign.*;

public final class TreeSitterHlsplaylist {
    ...
    private static final TreeSitterHlsplaylist INSTANCE = new TreeSitterHlsplaylist();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterHlsplaylist() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_hlsplaylist");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-hlsplaylist");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-hlsplaylist");
